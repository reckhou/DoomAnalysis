	...
	if len(params) >= 2 {
		...
		if pat == "get" {
			...
			if result {
				...
				contentSrc := string(reqContent)
				dumpProcessChan <- 1
				go dumpfile.ProcessDumpFile(dumpProcessChan, proname, contentSrc, lianyun)
			} else {
