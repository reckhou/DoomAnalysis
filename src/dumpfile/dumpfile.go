...
func (info *DumpFileInfo) GenNdkDumpInfo() {
	context := file.ReadFile("./dump/" + info.info_["version"] + "/" + info.file_name_ + ".info")
...
func (info *DumpFileInfo) GenNdkfile() {
	...
	*/
	...
	cmd := exec.Command("/bin/sh", "./gen_ndk_info.sh", info.info_["version"], info.file_name_+".ndk")
	b, err := cmd.Output()
	if err != nil {
...
func (info *DumpFileInfo) GenDbInfo() {
	context := file.ReadFile("./dump/" + info.info_["version"] + "/" + info.file_name_ + ".ndk.info")
	//context := file.ReadFile("./a.txt.info")
	start_pos := 0
	...
	db.CreateDB(info.info_["version"], info_key, info_str)
}
