...
type CheckDumpMysql struct {
	db *sql.DB
}

var sql_instance *DumpMysql
var sql_check_instance *CheckDumpMysql
var check_sql bool

type dbMemInfo struct {
...
func Init() (*DumpMysql, error) {
	if sql_instance == nil {
		...
		sql_instance.db = db
		sql_instance.db.SetMaxOpenConns(20)
		check_sql = true
	}
...
func (test *DumpMysql) AddInfo(pro string, ver string, address string, info string, uuid string, lianyun string) {
	...
	stmt1, stmt_err := test.db.Prepare("update " + pro + " set count=?,filelist=? where address=? and version =?")
	if stmt_err != nil {
		log.Println("Prepare :", stmt_err.Error())
		return
	}
	defer stmt1.Close()
	for rows.Next() {
		...
		if err == nil {
			...
			if result, err := stmt1.Exec(count_val, filelist_val, address, ver); err == nil {
...
func GerVersionList(pro string) string {
	...
	return_val = return_val + "<a href=\"?pat=allversion&pro=" + pro + "\">统一查询dump数量</a><br>"
	for rows.Next() {
...
func GetAllVersionList(pro string) string {
	...
	// 排序输出
	select_sql := "select id,address,count,ndk,filelist,lastupdate,version from " + pro + " order by count desc"
	select_rows, err := test.db.Query(select_sql)
	defer select_rows.Close()
	if err != nil {
		log.Println(err.Error())
	...
	return_val = return_val + "<th align=\"right\">Version</th>\n"
	return_val = return_val + "<th align=\"right\">COUNT </th>\n"
	return_val = return_val + "<th align=\"center\">INFO</th>\n"
	return_val = return_val + "<th align=\"center\">BREAKPAD</th>\n"
	return_val = return_val + "<th align=\"center\">TIME</th>\n"
	return_val = return_val + "</tr>\n"
	...
	index_val := 1
	for select_rows.Next() {
		if err := select_rows.Scan(&id, &address, &count_val, &ndk_val, &filelist_val, &lastupdate, &ver); err == nil {
			id_val := strconv.Itoa(id)
			count_id_val := strconv.Itoa(count_val)
			...
			return_val = return_val + "<tr>\n"
			return_val = return_val + "<th align=\"left\"><div " + color + ">" + id_val + "</div></th>\n"
			return_val = return_val + "<th align=\"right\"><div " + color + ">" + ver + "</div></th>\n"
			return_val = return_val + "<th align=\"right\"><div " + color + ">" + count_id_val + " </div></th>\n"
			return_val = return_val + "<th align=\"left\"><div " + color + ">" + ndk_val + " </div></th>\n"
			return_val = return_val + "<th align=\"left\">" + info_val + " </th>\n"
			return_val = return_val + "<th align=\"left\">" + lastupdate + " </th>\n"
			return_val = return_val + "</tr>\n"
			index_val++
		}
	}
